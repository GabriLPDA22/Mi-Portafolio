...
/* ------------------------------------------------------------------ */
/* Pila de proyectos (solo escritorio xl)                              */
/* ------------------------------------------------------------------ */

export function initProjectsStack() {
  const container = document.querySelector<HTMLElement>('.projects-stack');
  // En móvil/tablet las tarjetas van en flujo normal: sin trabajo por frame.
  if (!container || prefersReducedMotion() || window.innerWidth < 1280) return;

  const cards = [...container.querySelectorAll<HTMLElement>('.project-card')];
  const contents = [...container.querySelectorAll<HTMLElement>('.project-card-content')];
  if (cards.length < 2) return;

  let active = false;
  let naturalTops: number[] = [];
  const pins = cards.map(() => 0);
  const incoming = contents.map(() => 1.05);
  const outOp = contents.map(() => 1);
  const outSc = contents.map(() => 1);

  const measure = () => {
    const scrollY = window.scrollY;
    naturalTops = cards.map((card, i) => card.getBoundingClientRect().top + scrollY - pins[i]);
  };
  ...
  const loop = createIdleRafLoop((dt) => {
    ...
    cards.forEach((card, i) => {
      ...
      const nextTop = isLast ? naturalTop : naturalTops[i + 1];
      const pinDistance = Math.max(0, nextTop - naturalTop);
      const pinY = Math.min(pinDistance, Math.max(0, scrollY - (naturalTop - headerOffset)));
      if (Math.abs(pinY - pins[i]) > 0.1) moving = true;
      pins[i] = pinY;
      card.style.transform = `translate3d(0, ${pinY.toFixed(2)}px, 0)`;

      if (i === 0) return;
      const viewportTop = naturalTop - scrollY;
      const p = Math.min(1, Math.max(0, (vh - viewportTop) / (vh - vh * 0.5)));

      // Tarjeta que entra: de 1.05 a 1
      const inTarget = 1.05 - 0.05 * p;
      incoming[i] += (inTarget - incoming[i]) * lag;
      if (Math.abs(incoming[i] - inTarget) > 0.001) moving = true;

      // Tarjeta anterior: se aleja y se apaga
      const opT = 1 - 0.9 * p;
      const scT = 1 - 0.15 * p;
      outOp[i - 1] += (opT - outOp[i - 1]) * lag;
      outSc[i - 1] += (scT - outSc[i - 1]) * lag;
      if (Math.abs(outOp[i - 1] - opT) > 0.001 || Math.abs(outSc[i - 1] - scT) > 0.001) moving = true;
    });

    // Igual que la versión anterior: las tarjetas con sucesora siguen la escala de salida;
    // solo la última usa la escala de entrada.
    contents.forEach((content, i) => {
      const isLast = i === contents.length - 1;
      content.style.transform = `scale(${(isLast ? incoming[i] : outSc[i]).toFixed(4)})`;
      if (!isLast) content.style.opacity = outOp[i].toFixed(3);
    });
  ...
  window.addEventListener('scroll', () => active && loop.kick(), { passive: true });
