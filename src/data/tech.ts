...
export interface Tech {
  ...
  path?: string;
  /** Color de marca legible sobre el fondo claro del sitio. */
  color: string;
...
/** Las marcas con color casi blanco se pintan en tinta para que se lean sobre el fondo papel. */
const readable = (hex: string) => {
  const [r, g, b] = [0, 2, 4].map((i) => parseInt(hex.slice(i, i + 2), 16));
  const luminance = (0.2126 * r + 0.7152 * g + 0.0722 * b) / 255;
  return luminance > 0.85 ? '#121212' : `#${hex}`;
};
